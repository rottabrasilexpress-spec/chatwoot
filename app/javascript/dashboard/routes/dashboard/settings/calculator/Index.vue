...
import CalculatorResizablePanel from './CalculatorResizablePanel.vue';
import {
  canViewCalculator,
...
const activeBackgroundId = ref('amber');
const customBackgroundColor = ref('#fff7ed');
const hasLoadedLayoutPreferences = ref(false);
const placeAutocompleteListeners = [];

const selectedBackground = computed(() => {
  if (activeBackgroundId.value === 'custom') {
    return {
      id: 'custom',
      label: 'Personalizada',
      color: normalizeHexColor(customBackgroundColor.value) || '#fff7ed',
    };
  }

  return (
    calculatorBackgroundPresets.find(
      background => background.id === activeBackgroundId.value
    ) || calculatorBackgroundPresets[0]
  );
});

const calculatorCanvasStyle = computed(() => ({
  backgroundColor: selectedBackground.value.color,
}));

const calculatorCanvasTextColor = computed(() =>
...
const backgroundStorageKey = computed(
  () => `rotta-calculator:${accountId.value || 'default'}:background`
);

const selectBackground = background => {
  activeBackgroundId.value = background.id;
  customBackgroundColor.value = background.color;
};

const selectCustomBackground = event => {
...
const resetCalculatorLayout = () => {
  activeBackgroundId.value = 'amber';
  customBackgroundColor.value = calculatorBackgroundPresets[0].color;
  window.dispatchEvent(
    new CustomEvent('rotta-calculator-reset-layout', {
      detail: {
        storagePrefix: calculatorPanelStorageKey(''),
      },
    })
  );
};
...
watch([activeBackgroundId, customBackgroundColor], () => {
  if (!hasLoadedLayoutPreferences.value || typeof window === 'undefined')
    return;

  window.localStorage.setItem(
    backgroundStorageKey.value,
    JSON.stringify({
      id: activeBackgroundId.value,
      color: customBackgroundColor.value,
    })
  );
});
...
onMounted(() => {
  try {
    const savedBackground = JSON.parse(
      window.localStorage.getItem(backgroundStorageKey.value)
    );
    if (
      savedBackground?.id === 'custom' &&
      normalizeHexColor(savedBackground.color)
    ) {
      activeBackgroundId.value = 'custom';
      customBackgroundColor.value = normalizeHexColor(savedBackground.color);
    } else if (
      calculatorBackgroundPresets.some(
        background => background.id === savedBackground?.id
      )
    ) {
      activeBackgroundId.value = savedBackground.id;
    ...
    // A blocked or malformed preference falls back to the safe pastel default.
  } finally {
    hasLoadedLayoutPreferences.value = true;
  }
...
<template>
  <SettingsLayout>
    ...
    <template #body>
      <div
        v-if="canView"
        class="grid grid-cols-1 gap-3 p-2 mt-4 rounded-2xl xl:gap-4 xl:p-3 xl:grid-cols-[minmax(0,0.92fr)_minmax(24rem,1.08fr)]"
        :style="calculatorCanvasStyle"
      >
        <section
          class="flex flex-col gap-3 p-3 border rounded-xl border-n-weak bg-n-solid-1 xl:col-span-2"
          data-testid="calculator-layout-toolbar"
        >
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div class="flex items-start gap-3">
              <div
                class="flex items-center justify-center flex-shrink-0 rounded-xl size-10 bg-n-amber-3 text-n-amber-11"
              >
                <Icon icon="i-lucide-panels-top-left" class="size-5" />
              </div>
              <div>
                <h2 class="text-base font-semibold text-n-slate-12">
                  Ajuste visual da calculadora
                </h2>
                <p class="mt-1 text-sm text-n-slate-11">
                  Escolha um fundo pastel e arraste as bordas laranja dos cards
                  para alinhar cada área.
                </p>
              </div>
            </div>
            <span
          ...
          <div class="flex flex-wrap items-center gap-2">
            <div
              ...
              role="group"
              aria-label="Cores do fundo da calculadora"
            >
              <button
                v-for="background in calculatorBackgroundPresets"
                :key="background.id"
                type="button"
                class="flex items-center gap-2 px-2.5 py-1.5 text-xs font-semibold border rounded-lg border-n-weak text-n-slate-11 hover:border-n-brand"
                :class="{
                  'ring-2 ring-n-brand ring-offset-1':
                    activeBackgroundId === background.id,
                }"
                :aria-label="`Usar fundo ${background.label}`"
                :aria-pressed="activeBackgroundId === background.id"
                @click="selectBackground(background)"
              >
                <span
                  class="border rounded-full size-4 border-n-strong"
            ...
            <label
              class="inline-flex items-center gap-2 px-2.5 py-1.5 text-xs font-semibold border rounded-lg cursor-pointer border-n-weak text-n-slate-11 hover:border-n-brand"
              :class="{
                'ring-2 ring-n-brand ring-offset-1':
            ...
            >
              Personalizar
              <input
                class="w-7 h-7 p-0 overflow-hidden border-0 rounded cursor-pointer"
                type="color"
                :value="customBackgroundColor"
                aria-label="Escolher uma cor personalizada para o fundo"
                @input="selectCustomBackground"
              />
            </label>
            <span class="text-xs text-n-slate-11">
              {{ selectedBackground.label }} · salvo neste navegador
            </span>
            <button
              type="button"
              class="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-semibold border rounded-lg border-n-weak text-n-slate-11 hover:border-n-brand hover:text-n-brand"
              data-testid="calculator-reset-layout"
              @click="resetCalculatorLayout"
            >
              <Icon icon="i-lucide-rotate-ccw" class="size-3.5" />
              Restaurar layout
            </button>
          </div>
        </section>
        ...
        >
          <div class="flex min-w-0 flex-col gap-3">
            <CalculatorResizablePanel
              :storage-key="calculatorPanelStorageKey('reading')"
              label="Central de leitura"
              :min-width="320"
              :min-height="260"
              data-testid="calculator-resizable-reading"
            >
              <div
            ...
            <div class="flex flex-col gap-3">
              <CalculatorResizablePanel
                :storage-key="calculatorPanelStorageKey('freight')"
                label="Dados do frete"
                :min-width="320"
                :min-height="180"
                data-testid="calculator-resizable-freight"
              >
                <section
              ...
              <CalculatorResizablePanel
                class="order-3"
                :storage-key="calculatorPanelStorageKey('inventory')"
                label="Inventário e cubagem"
                :min-width="320"
                :min-height="220"
                data-testid="calculator-resizable-inventory"
              >
                <section
                  class="flex flex-col self-start gap-3 p-3 border-l-4 rounded-xl border-n-teal-5 bg-n-teal-1/40"
                >
                  <div>
                    <h2 class="text-base font-semibold text-n-slate-12">
              ...
              <CalculatorResizablePanel
                class="order-2"
                :storage-key="calculatorPanelStorageKey('services')"
                label="Serviços adicionais"
                :min-width="320"
                :min-height="300"
                data-testid="calculator-resizable-services"
              >
                <section class="flex flex-col self-start gap-3">
          ...
          <div class="flex min-w-0 flex-col gap-3">
            <CalculatorResizablePanel
              :storage-key="calculatorPanelStorageKey('route-board')"
              label="Mapa e rota"
              :min-width="360"
              :min-height="420"
              :max-height="1100"
              data-testid="calculator-resizable-route-board"
            >
            ...
            <CalculatorResizablePanel
              v-if="result"
              :storage-key="calculatorPanelStorageKey('result')"
              label="Resultado da calculadora"
              :min-width="360"
              :min-height="420"
              data-testid="calculator-resizable-result"
            >
              <section
                  ...
                  <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                    ...
                    >
                      <div class="flex items-center justify-between gap-2">
                        ...
                        <span class="text-xs font-semibold text-n-slate-11">
                          +{{ inventoryAudit.disassembled.marginPercent }}%
                          margem
                        </span>
                      </div>
                      <p class="mt-2 text-xl font-bold text-n-slate-12">
                        {{ inventoryAudit.disassembled.auditedM3.toFixed(3) }}
                        m³
                      </p>
                      <p class="mt-1 text-xs text-n-slate-11">
                        Veículo sugerido:
                ...
                >
                  <div class="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p class="mt-1 text-xs text-n-slate-11">
                        Seis faixas espelhadas do Hub. O motor aplica margem,
                        piso mínimo de R$ 600 no frete e ajuste manual por
                        quilômetro.
                      </p>
                    </div>
                  ...
                  <div class="flex flex-wrap gap-3 text-xs text-n-slate-11">
                    ...
                    <span
                      >Serviços:
                      {{ money(result.pricing.services.total) }}</span
                    >
