...
## Planejamento — Pergunte para IA — 13/09/2026

- Objetivo: criar no sidebar do Chatwoot uma área interna chamada `Pergunte para IA`, exclusiva para agentes autorizados, com respostas baseadas no histórico completo de clientes e conversas.
- Interface futura: chat interno, respostas com evidências, data, etiquetas, resumo, grau de confiança e cards clicáveis com o link direto da conversa. Nenhuma resposta será enviada ao cliente.
- Arquitetura recomendada: módulo nativo no Chatwoot com uma interface pequena (`perguntar`) e implementação profunda de busca, filtros, histórico e geração; DeepSeek `deepseek/deepseek-v4-flash-0731` via endpoint compatível com OpenRouter; N8N opcional, não obrigatório para o núcleo.
- Recuperação: busca estruturada por conta, contato, mensagem, etiqueta, status e período; índice atualizado por eventos de mensagem/conversa; contexto relevante enviado ao modelo, sem despejar todo o banco em cada pergunta.
- Segurança: autorização server-side por conta/agente, chave somente no servidor, respostas somente na área interna, sem UAZAPI, sem `Message#create` e sem alterações na primeira versão.
- Primeira versão recomendada: somente leitura. Ações futuras — etiqueta, arquivar, enviar mensagem — deverão ter ferramentas separadas e confirmação explícita.
- Custos de referência: OpenRouter informa US$ 0,05/M tokens de entrada e US$ 0,16/M de saída para o modelo; com contexto limitado, a estimativa é baixa. O custo de Captain/licença do Chatwoot é separado e não deve ser presumido como necessário para o módulo customizado.
- Estado: planejamento documentado; nenhum código, workflow N8N, deploy, credencial, conversa, mensagem ou dado persistente foi alterado. Implementação condicionada à aprovação do usuário.
- Linhas conectadas: [[Chatwoot Rotta — contexto e estado]] ↔ [configuração LLM](C:\Users\User\Documents\Codex\rotta-custom-v1-source\config\initializers\ai_agents.rb) ↔ [rotas](C:\Users\User\Documents\Codex\rotta-custom-v1-source\config\routes.rb) ↔ [GitHub rotta-custom-v1](https://github.com/rottabrasilexpress-spec/chatwoot/tree/rotta-custom-v1).

## Planejamento revisado — IA global de pendências e compartilhamento — 13/09/2026

- Evidência atual: já existe `Pergunte para IA` dentro da conversa, com thread por conversa, memória Redis, workflow n8n separado, contexto textual e ferramentas controladas. A nova entrega deve evoluir essa base, não duplicá-la nem alterar o Follow-up.
- Escopo novo: módulo global no sidebar, inicialmente visível somente para Kelvin, com botão `Espelhar para agentes`. O compartilhamento será uma concessão server-side revogável, não apenas esconder/mostrar o item com CSS.
- Otimização: cada conversa terá um snapshot incremental (`resumo`, `intenção`, `estado pendente`, `motivo`, `data desde`, `últimas evidências`, `confiança` e `última mensagem`). Perguntas globais consultam snapshots e poucos trechos relevantes; o histórico completo só será carregado sob demanda para uma conversa específica.
- Gatilhos: mensagens como `aguardar financeiro`, `enviar para o financeiro`, `retorno do setor`, `vou verificar e retornar`, pergunta do agente sem resposta do cliente, pergunta do cliente sem resposta efetiva e mudança de status/etiqueta. O detector deverá exigir evidência, ser idempotente e encerrar a pendência quando houver resposta, resolução ou reabertura coerente.
- Estados planejados: `aguardando_financeiro`, `aguardando_cliente`, `aguardando_agente`, `aguardando_documento`, `resolvido` e `revisao_manual`. O status nativo do Chatwoot será combinado com a última evidência textual, sem depender de uma única frase.
- Interface global: resposta do assistente com evidências e cards de clientes; cada card terá nome, etiqueta, status, tempo pendente, motivo, trecho-fonte, confiança, `Abrir conversa`, `Ver evidência` e `Marcar resolvido`. A navegação preservará a consulta e oferecerá retorno direto ao painel.
- Comandos: busca, filtros, etiquetas, remoção de etiquetas, status, atribuição, nota privada, mensagem pública e resolução serão ferramentas explícitas e autorizadas. Ações com efeito externo terão prévia, confirmação, idempotência e auditoria; nunca haverá SQL ou execução genérica exposta ao modelo.
- N8N: pode receber eventos incrementais para análise assíncrona, mas a leitura, autorização, snapshot, cards e ações devem permanecer no Chatwoot. O workflow atual por conversa continuará separado; recomenda-se um workflow global somente para classificação/projeção, se necessário.
- Testes obrigatórios: gatilho financeiro, cliente aguardando resposta, cliente respondido, resolução que remove o card, múltiplos clientes, duplicidade de eventos, Kelvin sem compartilhamento, concessão/revogação para outro agente, isolamento entre contas, abrir/voltar preservando a consulta e nenhuma pergunta gerando mensagem pública automaticamente.
- Estado: planejamento revisado, sem implementação, sem deploy e sem alteração de dados. A execução continua condicionada à aprovação do usuário.

## Vigésima terceira rodada — implementação inicial da IA global isolada — 13/09/2026

- A autorização foi confirmada e o módulo global começou separado do `Pergunte para IA` por conversa. Nenhum arquivo do fluxo `ConversationAi`, nenhum webhook N8N e nenhuma mensagem do Chatwoot foram alterados.
- Persistência própria criada: `global_ai_threads` e `global_ai_messages`, com conta/agente, histórico interno e validação dos campos gravados. O acesso inicial é Kelvin por e-mail/configuração de proprietário; o compartilhamento para agentes usa `rotta_global_ai_shared_user_ids`, é revogável e validado no servidor.
- Backend criado em `GlobalAiAssistant`: política de acesso, contexto compacto por conta, busca por contato/mensagem, status, etiquetas, evidências recentes e resumo de indicadores; geração direta pela configuração RubyLLM com o modelo confirmado `deepseek/deepseek-v4-flash-0731`, sem N8N.
- Ações isoladas e auditadas: adicionar/remover etiqueta, alterar status, criar nota privada, enviar mensagem pública e atribuir agente. A rota exige confirmação explícita para efeitos operacionais e nunca executa SQL ou comando arbitrário vindo da LLM.
- Interface criada no sidebar em `Pergunte para IA`: painel amplo, conversa persistida, cards selecionáveis, evidência, link para abrir conversa, retorno por histórico do navegador, marcar resolvida e compartilhamento para agentes. A janela não é o Copilot da conversa.
- Deploy ainda não iniciado nesta etapa porque a revisão Ruby/Rails precisa ocorrer no container EasyPanel; o host Windows não possui Ruby/Bundler e o Docker Desktop está indisponível localmente. O build Vite local passou com `5099` módulos; Vitest focalizado de visibilidade passou `3/3`; detector Impeccable retornou `[]`; `git diff --check` passou.
- Pendência técnica explícita: o Follow-up atual mantém o agendamento em um painel externo. Nesta primeira fatia a IA lê etiquetas/status e evidências; alterar horário/cancelar o job do Follow-up exigirá um adaptador nativo ou a aprovação de continuar usando a ponte existente. Isso não bloqueia o chat global, mas não deve ser apresentado como concluído.
- Arquivos principais: `app/services/global_ai_assistant/`, `app/controllers/api/v1/accounts/global_ai/`, `app/models/global_ai_*`, `app/javascript/dashboard/routes/dashboard/settings/globalAi/`, `app/javascript/dashboard/api/globalAi.js`, migration `20260913000000_create_global_ai_assistant.rb`, `config/routes.rb` e `docker/Dockerfile.overlay`.
- Linhas conectadas: [[Chatwoot Rotta — contexto e estado]] ↔ [plano global](C:\Users\User\Documents\Codex\rotta-custom-v1-source\docs\audits\gauntlet-2026-09-12-calculator-contact.md) ↔ [plano por conversa](C:\Users\User\Documents\Codex\rotta-custom-v1-source\docs\plans\pergunte-para-ia-conversa-2026-09-10.md) ↔ [GitHub rotta-custom-v1](https://github.com/rottabrasilexpress-spec/chatwoot/tree/rotta-custom-v1) ↔ [Chatwoot](https://n8nsaas-chatwoot-rotta.u9nqzz.easypanel.host/app/accounts/1/ask-ai).

## Vigésima quarta rodada — memória do chat global e evidências ampliadas — 13/09/2026

- O chat global agora reaproveita até 12 mensagens do próprio thread entre perguntas, mantendo continuidade sem misturar o histórico de outra conversa ou conta.
- O prompt recebe a memória separada das evidências da conta e instrui a LLM a tratar fatos do contexto como fonte principal, sem alegar ações que não foram confirmadas pela aplicação.
- A coleta de fontes textuais foi ampliada para evidências de financeiro, aguardas, retornos e documentos, mantendo o limite de 80 registros para preservar latência.
...
## Trigésima segunda rodada — auditoria live do perfil da mudança — 2026-09-14

- Escopo alinhado ao vídeo `C:\Users\User\AppData\Local\Packages\Microsoft.ScreenSketch_8wekyb3d8bbwe\TempState\Recordings\20260914-1208-56.8000895.mp4`: ler a conversa real, preencher o perfil sem mensagem externa, mostrar os campos completos e manter os dados após recarregar.
- Diagnóstico: o orçamento estruturado usava o marcador real `🪛 *Desmontagem:* ❌ NÃO`/`🪛 *Montagem:* ❌ NÃO`; o parser aceitava apenas outras variações de Markdown. A correção passou a aceitar os formatos com ícone, asteriscos antes/depois do rótulo e a remover o asterisco residual do valor.
- Commits publicados em `rotta-custom-v1`: `2960c1723`, `edda322ac` e `776241dc6` (final). Deploy concluído no EasyPanel; a URL live voltou a HTTP `200`.
- Teste real autenticado na conversa `2082`: o botão preencheu origem `Patos, PB, Brasil`, destino `Aracruz, ES, Brasil`, data `30/10/2026`, valor `R$ 3.655,37`, os 10 itens com quantidades, ajudantes `0/0`, montagem `❌ NÃO`, desmontagem `❌ NÃO` e observações.
- Persistência: após F5, todos os campos permaneceram exatamente preenchidos. O runner Rails confirmou `deterministic_profile_complete? = true`, portanto a cotação completa não faz chamada redundante ao modelo.
- Segunda execução consecutiva no mesmo navegador: aproximadamente `7,3 s`, sem perda dos campos e sem retorno vazio.
- Validações: Vitest focalizado `2/2`, `git diff --check`, health live e fluxo visual passaram. RSpec não foi executado no host Windows por ausência de Ruby/Bundler; a lógica Rails foi validada no container EasyPanel.
- Segurança: somente o perfil interno do contato de teste foi atualizado; não houve envio de mensagem, alteração de etiqueta/status/follow-up, arquivamento ou ação no WhatsApp.
- Linhas conectadas: [[Chatwoot Rotta — contexto e estado]] ↔ [ledger](C:\Users\User\Documents\Codex\rotta-custom-v1-source\docs\audits\gauntlet-2026-09-12-calculator-contact.md) ↔ [GitHub rotta-custom-v1](https://github.com/rottabrasilexpress-spec/chatwoot/tree/rotta-custom-v1) ↔ [EasyPanel](https://easypanel.via-cargo.com/projects/n8nsaas/compose/chatwoot-rotta/deployments) ↔ [Chatwoot 2082](https://n8nsaas-chatwoot-rotta.u9nqzz.easypanel.host/app/accounts/1/conversations/2082).
