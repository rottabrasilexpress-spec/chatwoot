...
- A primeira tentativa de publicação da tradução terminou `Success`, mas o manifesto passou a referenciar 14 bundles Vite não versionados, causando `404` e tela branca. Os arquivos referenciados foram incluídos no commit `ad452c07` (`fix(deploy): include all manifested Vite assets`) e enviados à branch `rotta-custom-v1`; o segundo deploy do EasyPanel também terminou `Success`. O alerta de containers órfãos foi observado, mas nenhum foi removido.
- Validação final: `/health`, `/app/login`, `dashboard-DQB8Vd7z.js`, `DashboardIcon-nI8fSl3G.js` e `ArticleEditor-Ce5NsIy4.js` responderam HTTP `200`. Após reabrir a caixa `WhatsApp Rotta` pela lista, a interface exibiu o rótulo e texto de ajuda em português; o toggle permaneceu `0` (desligado). O estado persistido não foi alterado e nenhuma mensagem real foi enviada.
- HEAD GitHub registrado: `ad452c07`; a correção do locale regional está em `06514765`. Vitest focalizado `25/25`, ESLint, Prettier, asserção de locale e build Vite aprovados; `verify:manifest-assets` validou 240 referências e todas estão versionadas. O RSpec do adaptador continua pendente por Ruby/Bundler indisponíveis localmente.
