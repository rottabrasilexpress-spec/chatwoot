...
## Checkpoint 30 — popup independente do Caio e solicitação de atenção validados live — 12/09/2026

- A lista de agentes live confirmou `Caio Mazine` como o destinatário configurado. Foi aberta uma sessão separada autenticada como Caio e a conversa `#2143` foi carregada nos dois perfis.
- Pela sessão de Kelvin, a etiqueta `Caio Atenção` foi adicionada temporariamente. Na sessão do Caio apareceu, no canto inferior direito, o card `CAIO ATENÇÃO` com `~Kelvin Martins`, `+5511965927865` e a explicação de que a etiqueta foi adicionada; o card não apareceu na sessão de Kelvin. A etiqueta foi removida e o contador convergiu de `2` para `1` após a atualização live; nenhum texto foi enviado ao cliente.
- Pela sessão de Kelvin, o menu contextual exibiu `Solicitar Atenção` e `Copiar link da conversa`, sem `Reabrir conversa`, `Deixar pendente` ou `Fechar conversa`. Ao acionar `Solicitar Atenção`, Kelvin recebeu somente o toast `Solicitação enviada para o agente Caio.`; na sessão do Caio apareceu o card `SOLICITAÇÃO DE ATENÇÃO` com cliente, telefone e a mensagem `Este cliente é um possível fechamento e precisa de atenção.` Não houve mensagem pública/privada ou envio para o WhatsApp.
- O caminho de áudio foi validado por código e teste: `CaioAttentionAlertAudio` cria `/audio/dashboard/bell.mp3`, força volume `1` independentemente da preferência normal e mantém o card quando o navegador rejeita autoplay. O asset live respondeu HTTP 200 `audio/mpeg`; a reprodução audível física não é mensurável pela árvore de acessibilidade, portanto essa parte permanece evidenciada pelo código/teste, não por medição acústica.
...
## Checkpoint 33 — auditoria de testes e ambiente publicado — 12/09/2026

- Suíte frontend focalizada: `7` arquivos e `52/52` testes aprovados. Foram cobertos contadores de etiquetas, normalização de `FINALIZADOS`, atualização Action Cable, fila de follow-up, operações de etiquetas, API de conversas e o parâmetro `per_page: 50`.
- Integridade: `verify:manifest-assets` aprovou `240` assets; `git diff --check` aprovou; ESLint terminou com `0 errors` e somente o aviso preexistente de chave i18n dinâmica em `ChatList.vue`.
- Saúde live: `/api` retornou HTTP `200`, Chatwoot `4.17.0`, `queue_services: ok` e `data_services: ok`. A captura de rede autenticada confirmou requisições do dashboard e das filas de etiquetas com `page=1&per_page=50`, todas HTTP `200`. A lista visual mostra somente os itens no viewport por virtualização; isso não invalida o lote de 50 recebido pela API.
- Menu contextual live: `Enviar para FINALIZADOS` presente e `ADIAR` ausente. Console do navegador não registrou erros durante a navegação.
- Lacuna de execução: Ruby/Bundler não estão instalados e o daemon Docker não está disponível nesta máquina; portanto os specs Rails de `Label` e `ConversationFinder` não foram executados localmente. O código e os specs permanecem prontos para execução no ambiente Rails.
- Evidência pendente encontrada sem alteração de dados: a conversa `#2269` está resolvida (`Reabrir`) e conserva no histórico eventos de resolução e reabertura automática, mas o menu atual mostra `0` etiquetas selecionadas e a fila `FINALIZADOS` está vazia. Isso pode ser consequência do fixture ter sido manipulado durante as provas anteriores, mas não deve ser tratado como aprovação de persistência sem um novo teste isolado add→resolve→reabrir por mensagem→verificar etiqueta. Nenhuma alteração foi feita nesta auditoria.
- Classificação: frontend, bundle, saúde do deploy e lote inicial aprovados; backend Rails e persistência completa da etiqueta FINALIZADOS ficam condicionados à execução dos specs Rails e a uma nova prova isolada, sem reutilizar a conversa 2269.
- Linhas conectadas: [contexto no Obsidian](C:/Users/User/Documents/Obsidian/Meu%20Cofre/Meu%20Cofre/Projetos/Chatwoot%20Rotta/00%20-%20Contexto%20e%20estado.md) ↔ [pesquisa móvel](../research/chatwoot-mobile-connection-20260911.md) ↔ [GitHub rotta-custom-v1](https://github.com/rottabrasilexpress-spec/chatwoot/tree/rotta-custom-v1).

## Checkpoint 34 — refinamento visual, bundles publicados e auditoria de testes — 12/09/2026

- A gravação de `102,53 s` foi revisada e transcrita. O escopo confirmado foi: remover `Todas` do cabeçalho, retirar o atalho de `Arquivados` somente do cabeçalho (a opção da barra lateral permanece), manter `Ler tudo`, centralizar a busca, tornar a nova conversa semelhante ao WhatsApp e melhorar o player de áudio com velocidade e progresso laranja.
- O código foi ajustado somente nos componentes desse escopo. O commit `485f5af41` cobre cabeçalho, seletor de contato/telefone, composer inicial e player de áudio; os testes de regressão novos reproduziram os sintomas antes da correção e passaram depois.
- Vitest focalizado pós-correção: `4` arquivos, `42/42` testes aprovados. ESLint dos arquivos alterados: `0 errors` e somente `5` avisos de chaves i18n dinâmicas já existentes. O detector visual do Impeccable retornou `[]`; `verify:manifest-assets` confirmou `240` assets.
- A suíte completa executada diretamente pelo Vitest no Windows totalizou `4.362` testes: `4.329` aprovados e `33` falhas em `10` arquivos, incluindo `2` mismatches de snapshot. As falhas ficaram em testes preexistentes de timezone/locale e estados de store/macros/hotkeys; nenhum teste dos componentes alterados falhou. O script npm original usa `TZ=UTC` em sintaxe Unix, então a execução direta foi usada para obter o resultado real neste shell.
- O primeiro deploy do código confirmou o serviço saudável, mas ainda servia o bundle anterior. Foi então executado o build Vite e publicado o commit `5b3e5005b` com o manifesto e os assets novos. O segundo deploy terminou com `Success`; houve apenas a janela transitória esperada de reinício, depois `/api` voltou a HTTP `200`, Chatwoot `4.17.0`, `queue_services: ok` e `data_services: ok`.
- Prova live pós-bundle: o dashboard serviu `dashboard-DtrwM39M.js`; o cabeçalho não contém `Todas` nem o link de `Arquivados`, enquanto `Arquivados` continua corretamente na barra lateral. `Ler tudo`, busca, filtro e nova conversa permanecem presentes. A lista carregou automaticamente e exibiu `Todas as conversas carregadas` sem depender de scroll.
- Prova live somente leitura da nova conversa: ao abrir `Iniciar nova conversa`, apareceu inicialmente apenas `Para` com o seletor de contato; não apareceu o composer de mensagem nem `Via` desabilitado antes da seleção de contato. O modal foi descartado sem criar conversa e sem enviar mensagem.
- Nenhuma mensagem, etiqueta, conversa, inbox ou cliente foi alterado nesta rodada. O resultado aprovado é frontend + bundle + saúde live; a suíte completa continua com as falhas baseline registradas acima e não deve ser apresentada como 100% verde.
- Linhas conectadas: [contexto no Obsidian](C:/Users/User/Documents/Obsidian/Meu%20Cofre/Meu%20Cofre/Projetos/Chatwoot%20Rotta/00%20-%20Contexto%20e%20estado.md) ↔ [pesquisa móvel](../research/chatwoot-mobile-connection-20260911.md) ↔ [GitHub rotta-custom-v1](https://github.com/rottabrasilexpress-spec/chatwoot/tree/rotta-custom-v1).
