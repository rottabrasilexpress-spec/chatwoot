...
- Smoke pós-deploy: `/health` HTTP 200 (`{"status":"woot"}`) e `/app/login` HTTP 200. O branch GitHub `rotta-custom-v1` aponta para o mesmo SHA `65f8e7ae0c90a4276e8aa3ee5023883fc7eff9e3`.
...
- Trilha funcional publicada: `a8529b1b` (áudio, timeout, identidade e status), `beddad06` (busca local/remota), `68c44b37` (bundles Vite exigidos pelo overlay Docker) e `65f8e7ae` (fallback visual para contato sem nome). A atualização documental final será registrada no GitHub sem redeploy da aplicação.
